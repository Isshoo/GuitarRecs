...
const CollaborativeFilteringService = require("./collaborativeFiltering");
...
class EvaluationService {
   ...
   */
  static async evaluatePerformance(k = null) {
    // Get K value from config if not provided
    if (k === null) {
      k = await CollaborativeFilteringService.getKValue();
    }
    ...
    for (const rating of allRatings) {
      try {
        ...
        const prediction = await CollaborativeFilteringService.predictRating(rating.userId, rating.guitarId, k);
   ...
   */
  static async getEvaluationByUser(k = null) {
    const { details, mae, rmse, ...summary } = await EvaluationService.evaluatePerformance(k);
